...
function AppointmentRow({
  ...
  appt: Appointment;
  idx: number;
  onPrescribe: (a: Appointment) => void;
}) {
...
function SlotCard({
  ...
  appointments,
  onPrescribe,
}: SlotGroup & { onPrescribe: (a: Appointment) => void }) {
...
export default function DoctorAppointmentsPage() {
  ...
  const handlePrescribe = (appt: Appointment) => {
    const params = new URLSearchParams({
      ...
      patientUserId: appt.patient._id,
      apptDay: appt.day,
      apptStart: appt.timeSlot.startTime,
      apptEnd: appt.timeSlot.endTime,
    });
    if (appt.symptoms?.trim()) params.set("symptoms", appt.symptoms.trim());
    router.push(`/doctor/add-prescription?${params.toString()}`);
  };
